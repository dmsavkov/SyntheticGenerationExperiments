"""Exp 8: generate + Llama validate + probe filter (YES only)."""
...
def run(*, save: bool = True, rebuild_splits: bool = False, synthetic_cap: int = SYNTHETIC_CAP_DEFAULT) -> dict:
    ...
    exp = "exp08_validated"
    hyp = f"{HYPOTHESES_BASE} Exp8: validated synthetic (YES only for training)."
    cfg = base_config(
        experiment=exp,
        train_split="train_2k",
        eval_split="eval_1000",
        hypotheses=hyp,
        extra={"generation_mode": "label_failure", "validation_filter": "YES_only"},
    )
    ...
    yes_rows, _ = run_validation_pipeline(probe, synthetics, labels, session)
    probe2, train_s2 = train_probe(train_rows + yes_rows)

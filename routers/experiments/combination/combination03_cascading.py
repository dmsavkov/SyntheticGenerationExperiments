"""Combination 03: 3-iter cascading Gemini chat on holdout failures."""
...
def run(*, save: bool = True, rebuild_splits: bool = False) -> dict:
    ...
    hyp = f"{HYPOTHESES_BASE} Combination03: cascading chat ({COMBINATION_CASCADE_ITERS} iters)."
    cfg = ensemble_base_config(
        experiment="combination03_cascading",
        topic=COMBINATION_TOPIC,
        hypotheses=hyp,
        extra={"generation_mode": "cascade", "cascade_iters": COMBINATION_CASCADE_ITERS},
    )
    session = ExperimentSession("combination03_cascading", topic=COMBINATION_TOPIC)
    ...
    if save:
        ...
        if len(synth_by_iter) >= 2:
            ...
            sub_session = ExperimentSession(
                "combination03_cascading_iter23",
                topic=COMBINATION_TOPIC,
            )
    ...
    logger.info("Combination03 complete; variants={}", list(eval_variants.keys()))
